...
The `0.0.3` owner build reads Markdown from the active disposable vault while enabled. It uses one in-memory local Worker and compiles out daemon access, network access, credentials, settings profiles, and disk-cache persistence. Its index is discarded on unload and rebuilt after restart.
...
Release `0.0.2` remains frozen and contains the known truncated-result `invalid_request` defect corrected in `0.0.3`. Release `0.0.1` remains frozen as the previous fixture-only build. Neither is an equivalent active-vault fallback for `0.0.3`.

## Source and evidence

Corresponding source for release `0.0.3`:

- repository: <https://github.com/cybersader/kwiry>
- source tag: [`d5c-balanced-playground-0.0.3-r1-source`](https://github.com/cybersader/kwiry/tree/d5c-balanced-playground-0.0.3-r1-source)
- source commit: `bebd4a664a36aff7a0a5f5a44ccbb8bfeacce6eb`
- validated workflow: [GitHub Actions run `30711596241`](https://github.com/cybersader/kwiry/actions/runs/30711596241)

Release `0.0.3` corrects the truncated Text-versus-Balanced movement count that could make an otherwise valid comparison fail as `invalid_request`. It provides the three runtime files plus GPL, Apache-2.0, Emscripten, and exact Rust dependency license terms; third-party notices; `SHA256SUMS`; and a sanitized build attestation. The attestation records source identity, same-environment repeatability, embedded artifact identities, cache/network isolation, and known limitations without vault paths, note content, query content, or credentials.
...
The separate production 10,000-note/50-MiB Worker capacity regression remains unresolved and is tracked outside this field experiment. Release `0.0.3` must report existing limits honestly; it does not fix or bypass that regression.
